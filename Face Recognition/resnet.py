...
########################################################################
################## Building the Basic Residual Block ###################
...
# The Resnet block has four levels with different number of residual blocks in it.
...
class Resnet(nn.Module):
    def __init__(self, block, layers, num_classes, zero_init_residual=False, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)          # Changed kernel size from actual Resnet architecture
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False): # Function to make each of the four layers
...
def resnet(block, layers, num_classes):
    model = Resnet(block, layers, num_classes)
    return model

if __name__ == "__main__":
    model = resnet(BasicBlock, [2,2,2,2], 256) # Resnet18 (a total of 8 layers with 2 layers in each level)
